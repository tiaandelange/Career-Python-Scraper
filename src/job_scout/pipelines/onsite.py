...
def run_onsite_pipeline(repo: JobRepository, fx: FxService | None = None) -> PipelineStats:
    return run_pipeline(
        pipeline=PipelineName.ONSITE,
        adapters=iter_adapters(pipeline="onsite"),
        repo=repo,
        fx=fx,
        expected_work_mode=WorkMode.ONSITE,
    )
